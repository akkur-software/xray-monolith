...
CylinderShape::CylinderShape(const std::array<float, 9>& points)
{
	m_type = ParticleShapeType::Cylinder;
...
bool CylinderShape::IsWithin(const Pvector& vec) const
{
	...
	return (sqr <= m_outerRadiusSquare && sqr >= _sqr(m_innerRadius));
}

Pvector CylinderShape::GeneratePoint() const
{
...
void CylinderShape::Transform(const ParticleShapeBase* sourceShape, const Fmatrix& matrix)
{
...
void CylinderShape::TransformDir(const ParticleShapeBase* sourceShape, const Fmatrix& matrix)
{
