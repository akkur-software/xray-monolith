...
bool CylinderShapeController::IsWithin(const ParticleShape& shape, const Pvector& vec) const
{
	...
	return (sqr <= shape.OuterRadiusSquare && sqr >= _sqr(shape.InnerRadius));
}

ParticleShape CylinderShapeController::Transform(const ParticleShape& shape, const Fmatrix& matrix)
{
	auto newShape = shape;
...
ParticleShape CylinderShapeController::TransformDir(const ParticleShape& shape, const Fmatrix& matrix)
{
...
Pvector CylinderShapeController::GeneratePoint(const ParticleShape& shape) const
{
...
ParticleShape CylinderShapeController::Create(float p0, float p1, float p2, float p3, float p4, float p5, float p6, float p7, float p8)
{
    auto shape = ParticleShape();

	shape.Type = ParticleShapeType::Cylinder;
